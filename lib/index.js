...
export default function generate(webIdlString, implModuleName, implementsModuleNameGetter = defaultImplements) {
    var { theInterface, implemented } = getInterfaceAndImplemented(webIdlString);
    ...
    var transferCustomElementCallbacks = theInterface.extAttrs.some(xa => xa.name === "CustomElementCallbacks");
    var includeReflector = attributes.some(hasReflect);
    var hasNonReflectedAttributes = attributes.some(a => !hasReflect(a));
    var includeImpl = transferCustomElementCallbacks || operations.length > 0 || hasNonReflectedAttributes;
    var includeConversions = operations.length > 0 ||
            attributes.some(a => (!hasReflect(a) || !a.readonly) && !hasNoConversion(a));
    var implementedMap = new Map(implemented.map(name => [name, implementsModuleNameGetter(name)]));
    ...
    attributes.forEach(a => {
        ...
        var noConversion = a.extAttrs.some(xa => xa.name === "NoConversion");
        if (reflect.shouldReflect) {
            var getterTree = codeGen.reflectGetterTree(a.name, a.idlType.idlType, reflect.as, { noConversion });
            classTree.elements.push(getterTree);

            if (!a.readonly) {
                var setterTree = codeGen.reflectSetterTree(a.name, a.idlType.idlType, reflect.as, { noConversion });
    ...
    for (var mixinName of implemented) {
...
function getInterfaceAndImplemented(webIdlString) {
    ...
    var implementsNodes = idlTree.filter(n => n.type === "implements");
    var rest = idlTree.filter(n => n.type !== "implements");

    if (rest.length > 1) {
        throw new Error("IDL file must contain only a single interface, potentially with implements statements");
    }
    var theInterface = rest[0];
    ...
    var implemented = implementsNodes.map(n => {

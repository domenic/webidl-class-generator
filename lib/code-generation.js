...
export function moduleTree(className, { includeReflector = false } = {}) {
    var moduleTree = parseModule`export default class ${className} { }`;
...
export function reflectGetterTree(name, type) {
    return parsePropertyDefinition`get ${name}() { return reflector[${type}](this, ${name.toLowerCase()}); }`;
}

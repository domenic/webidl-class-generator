...
var { parseModule, parsePropertyDefinition, parseStatement }
    = require("traceur").get("codegeneration/PlaceholderParser");
...
export function methodTree(name, args) {
    ...
    // Be sure to create a new function body, instead of just overwriting `methodTree.body.statements`, since we
    // don't want to share a body among all instances. This is a consequence of the Traceur PlaceholderParser caching
    // and how we keep abusing it by mutating the resulting ASTs: https://github.com/google/traceur-compiler/issues/1407
    methodTree.body = createFunctionBody(args.map(arg => {
